...
class _ConnectionButtonState extends State<ConnectionButton> with SingleTickerProviderStateMixin {
      ...
      child: SizedBox(
        ...
        child: Consumer<VpnProvider>(
          builder: (context, value, child) {
            switch (value.vpnStage ?? VPNStage.disconnected.name) {
              case "connected":
                _controller.stop();
  ...
  Widget _connectingUI(BuildContext context, String stage) {
    return Stack(
      fit: StackFit.expand,
      children: [
        AnimatedBuilder(
  ...
  void _connectButtonClick(BuildContext context) {
    ...
    if (vpnProvider.vpnStage?.toLowerCase() != VPNStage.disconnected.name.toLowerCase()) {
